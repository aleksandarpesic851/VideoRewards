...
class requests extends db_connect
{
	...
	{
        $stmt = $this->db->prepare("INSERT INTO Completed(rid,request_from,dev_name,dev_man,gift_name,req_amount,points_used,date,username) SELECT rid,request_from,dev_name,dev_man,gift_name,req_amount,points_used,date,username FROM Requests WHERE rid = '$id'");
    ...
    public function ProcessingRequest($id)
	{
	...
	{
        $stmt = $this->db->prepare("UPDATE Requests SET status = 3 WHERE rid = '$id'");
