...
class settings extends db_connect
{
    ...
    function saveSettings($appname, $tagline, $package, $support_email, $webpanel_url, $company_name, $company_site, $company_phone, $company_email, $country, $income_activate, $admin_ratio,$firebase_key) {
		
		$set = new functions($this->db);
		
		$result = $set->updateConfigs($appname, 'APP_NAME');
		$result = $set->updateConfigs($tagline, 'APP_DESC');
		$result = $set->updateConfigs($package, 'PACKAGE_NAME');
		$result = $set->updateConfigs(time(), 'LAST_SAVE');
		$result = $set->updateConfigs($support_email, 'SUPPORT_EMAIL');
		$result = $set->updateConfigs($webpanel_url, 'WEB_ROOT');
		$result = $set->updateConfigs($company_name, 'COMPANY_NAME');
		$result = $set->updateConfigs($company_site, 'COMPANY_SITE');
		$result = $set->updateConfigs($company_phone, 'SUPPORT_PHONE');
		$result = $set->updateConfigs($company_email, 'COMPANY_EMAIL');
		$result = $set->updateConfigs($country, 'COMPANY_COUNTRY');
		$result = $set->updateConfigs($income_activate, 'INCOME_OVERVIEW');
		$result = $set->updateConfigs($admin_ratio, 'ADMIN_RATIO');
		$result = $set->updateConfigs($firebase_key, 'FIREBASE_API_KEY');
