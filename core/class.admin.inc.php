...
class admin extends db_connect
{
    ...
    public function padData($str, $isB = false)
    {
		$str = $isB ? $str : utf8_decode($str);
        $tdo = mcrypt_module_open('rijndael-128', ' ', 'cbc', "@@@@&&&&####$$$$");
		$set = new functions($this->db);
		$key = $set->getConfig('ACCESS_TOKEN');
		if(strlen($key) < 16){ $key = "tm58pal17padDt69"; }
        mcrypt_generic_init($tdo, $key, "@@@@&&&&####$$$$");
        $fd = mcrypt_generic($tdo, $str);
        mcrypt_generic_deinit($tdo);
        mcrypt_module_close($tdo);
        return $isB ? $fd : bin2hex($fd);
    }
